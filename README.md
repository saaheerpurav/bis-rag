...
> **Note on additional evaluation:** The table above is based solely on the **provided public test set (10 queries)**. Additional robustness tests on larger/custom query sets are documented in [`docs/extended_eval.md`](docs/extended_eval.md) for reference only. **Final scores are calculated exclusively on the organizer-provided test set using the rulebook metrics.**

---

## Judge Setup (3 steps, ~2 minutes)

> **Python 3.11 required.** `fastembed==0.4.2` does not support Python 3.12 or later. Use Python 3.11.x exactly.
...
- **Multi-channel delivery**: Web UI (Next.js + FastAPI), WhatsApp bot (Twilio + Whisper voice notes), React Native mobile app.
...
| No smartphone/PC access | WhatsApp bot accepts text and voice notes in Hindi or English |
...
| Voice transcription | OpenAI Whisper | WhatsApp voice note input |
